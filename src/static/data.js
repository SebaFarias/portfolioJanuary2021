
const data = {
  skills: [
    {
      label: 'Html 5 CSS 3',
      title: 'First Skill',
      content: 'This is an explanation for the First Skill',
      projects:[],
    },
    {
      label: 'Number 2',
      title: 'Second Skill',
      content: 'This is an explanation for the Second Skill',
      projects:[],
    },
    {
      label: '3',
      title: 'Third Skill',
      content: 'This is an explanation for the third Skill',
      projects:[],
    }],
  experience: [
    {
      company: "Aligare",
      role: "Jr .NET Dev",
      content: {
        description: "Proyecto para el Ministerio de Vivienda y Urbanismo",
        responsabilities: {
          
        },
      },
      initialDate: "Octubre 2018",
      endDate: "Agosto 2019",
    },
    {
      company: "SurtiTech",
      role: "Regional Manager",
      content: {
        description: " Surtitech presta el servicio de Telemetría de más de 3.000 centrales de GLP pertenecientes a Abastible a lo largo de todo Chile. Como Regional Manager, estaba a cargo de la operación de las centrales ubicadas entre Arica y Parinacota y la región de El Maule",
        responsabilities: {
          routes: "",
          providers: "",
          client: "",
          inventory: "",
          practicantes: "",
          procedures: "",
          repair: "",
          instalador: {
            installer: "Encargado de visitar centrales sin transmisión para identificar la falla en comunicación con un supporter de AIUT(Proveedor de los equipos) y repararla. ",
            supporter: "Encargado de visitar centrales sin dispositivos instalados para instalarlos ",
          },
        },
      },
      initialDate: "Octubre 2018",
      endDate: "Agosto 2019",
    },
  ],
  projects: [
    {
      name: "Gato",
      repository: "https://github.com/SebaFarias/Gato",
      live: "https://laughing-bohr-918a5d.netlify.app/",
      keywords: [ 'Front-End', 'Front End', 'Full Stack', 'Full-Stack' ],
      stack: [],
      description: "blah, blah, blah, blah, blah, blah, blah",
      thumbnail: 'https://i.postimg.cc/h4wzfh88/Gato.gif',
    },
    {
      name: "Gato Server",
      repository: "https://github.com/SebaFarias/GatoServer",
      live: "https://gato-server.herokuapp.com/ ",
      keywords: [ 'Back-End', 'Back End', 'Full Stack', 'Full-Stack' ],
      stack: [],
      description: "blah, blah, blah, blah, blah, blah, blah",
      thumbnail: 'https://i.postimg.cc/h4wzfh88/Gato.gif',
    },
    {
      name: "Algo Viewer",
      repository: "https://github.com/SebaFarias/sorting-viewer",
      live: "https://reverent-jepsen-d6c745.netlify.app/",
      keywords: ['react','SPA','Single Page Application','Front-End','Front End'],
      stack: [],
      description: "blah, blah, blah, blah, blah, blah, blah",
      thumbnail: 'https://i.postimg.cc/wB03HBTm/Algo-Viewer.gif',
    },
    {
      name: "Theme switcher",
      repository: "https://github.com/SebaFarias/Theme-switcher-Challenge",
      live: "https://cranky-villani-063a08.netlify.app/",
      keywords: ['react','SPA','Single Page Application','Front-End','Front End'],
      stack: [],
      description: "blah, blah, blah, blah, blah, blah, blah",
      thumbnail: 'https://i.postimg.cc/tgX7L6yb/Theme-Switcher.gif',
    },
    {
      name: "Ip Tracker",
      repository: "https://github.com/SebaFarias/IP-Tracker",
      live: "https://ip-tracker-psi.vercel.app/",
      keywords: ['SPA','Single Page Application','Front-End','Front End'],
      stack: [],
      description: "blah, blah, blah, blah, blah, blah, blah",
      thumbnail: 'https://i.postimg.cc/2SN17JKV/IP-Tracker.gif',
    },
    {
      name: "Chat App",
      repository: "https://github.com/SebaFarias/chat-app-callenge",
      live: "https://festive-bardeen-801143.netlify.app/",
      keywords: ['Front-End','Front End'],
      stack: [],
      description: "blah, blah, blah, blah, blah, blah, blah",
      thumbnail: 'https://i.postimg.cc/PrwLZhKg/Chat-App.gif',
    },
    {
      name: "Memorice Pokemon",
      repository: "https://github.com/SebaFarias/memorice-pokemon-cliente",
      live: "https://hungry-aryabhata-33c405.netlify.app/",
      keywords: [ 'Front-End', 'Front End', 'Full Stack', 'Full-Stack', 'react','SPA','Single Page Application'],      
      stack: [],
      description: "blah, blah, blah, blah, blah, blah, blah",
      thumbnail: 'https://i.postimg.cc/PrwLZhKg/Chat-App.gif',
    },
  ],  
}

export default data